...
export default function Sidebar({
	...
	return (
		<>
			<div
				className={`${isSidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-0"} z-2 absolute lg:relative flex flex-col h-full w-full border shadow-sm overflow-hidden bg-white border-stone-200 shadow-stone-950/5 max-w-[280px] transition-transform duration-300 ease-in-out`}
			>
				<div className="m-2 mx-4 mb-0 mt-3 h-max flex items-center justify-between">
					...
					<button
						className="hidden lg-block p-2 hover:font-bold hover:cursor-pointer"
				...
				<div className="mt-auto p-4 border-t border-stone-200 flex items-center justify-between">
					...
					<button
						className="p-2 hover:font-bold hover:cursor-pointer"
						onClick={onLogout}
					>
						<PowerIcon
							aria-hidden="true"
			...
			>
				<BookForm
					...
					onSave={(book) => handleAddBook(book)}
				/>
			</Drawer>
			<Toast
				isOpen={isToasAddOpen}
				title="Successfully saved!"
				message="The book has been added successfully"
				type="success"
				onClose={() => setIsToastAddOpen(false)}
			/>
			<Toast
				isOpen={error}
				title="Failed!"
