...
import { deleteBook, fetchBooks, updateBook } from "../../../services/models";
	...
	const handleFillterBook = (e: React.ChangeEvent<HTMLSelectElement>) => {
		...
		switch (filter) {
			case "all":
				...
				break;
			case "recent":
				const recentBooks = [...state.books].sort(
					(a: Book, b: Book) =>
				...
				break;
			case "increasing":
				const increasingBooks = [...state.books].sort(
					(a: Book, b: Book) => a.title.localeCompare(b.title),
				);
				setBooks(increasingBooks);
				break;
			case "decreasing":
				const decreasingBooks = [...state.books].sort(
					(a: Book, b: Book) => b.title.localeCompare(a.title),
				);
		...
		<>
			...
			>
				<BookForm
					...
					onSave={(book) => handleEdit(book)}
				/>
			</Drawer>
			<Toast
				isOpen={isToasDeleteOpen}
				title="Deleted!"
				message="The book has been deleted successfully"
				type="success"
				onClose={() => setIsToastDeleteOpen(false)}
			/>
			<Toast
				isOpen={isToasEditOpen}
				title="Successfully saved!"
				message="The book has been updated successfully"
				type="success"
				onClose={() => setIsToastEditOpen(false)}
			/>
		</>
	);
}
