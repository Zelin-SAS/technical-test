...
import logo from "../../../../assets/zbook-logo-0.svg";

export default function Footer({navigationRef }: {navigationRef: React.RefObject<HTMLDivElement | null>[]}) {
    ...
    useEffect(() => {
        const handleScroll = () => {
            if (window.scrollY > 0) {
                footerRefs.current?.classList.add('backdrop-blur-md');
            } else {
                footerRefs.current?.classList.remove('backdrop-blur-md');
            }
        };
    ...
    const handleNavigation = (e: React.MouseEvent<HTMLAnchorElement>) => {
        ...
        if (index !== null && !isNaN(Number(index))) {
            ...
            const element = navigationRef[Number(index)]?.current;
            if (element) {
    ...
    <footer className="bg-gray-800 text-white pt-12 pb-8 px-4" ref={footerRefs}>
        <div className="mx-auto px-4 container overflow-hidden flex flex-col lg:flex-row justify-between">
            ...
            <div className="w-2/3 block sm:flex text-sm mt-6 lg:mt-0">
                <ul className="text-gray-700 list-none p-0 font-thin flex flex-col text-left w-full">
                    <li className="inline-block py-2 px-3 text-white uppercase font-medium tracking-wide">Links</li>
                    {navigation.map((item, index) => (
                        <li key={item.id}>
                            <a data-id={index} onClick={handleNavigation} className={`inline-block py-2 px-3 text-gray-500 hover:cursor-pointer hover:text-white no-underline ${item.active ? 'text-white' : ''}`}>{item.name}</a>
                        </li>
                    ))}
                </ul>
                <ul className="text-gray-700 list-none p-0 font-thin flex flex-col text-left w-full">
        ...
        <div className="mt-4 pt-6 text-gray-600 border-t border-gray-800 text-center"> © 2025 Zebook. All rights reserved.</div>
    </footer>
  )
}