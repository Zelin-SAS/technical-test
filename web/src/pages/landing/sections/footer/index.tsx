...
export default function Footer({
	...
	return (
		<footer
			className="bg-gray-800 text-white pt-12 pb-8 px-4"
			ref={footerRefs}
		>
			<div className="mx-auto px-4 container overflow-hidden flex flex-col lg:flex-row justify-between">
				<a className="flex items-start">
					...
					<span className="ml-2 text-xl font-bold text-[#DCF763]">
						ZeBook
					</span>
				</a>
				<div className="w-2/3 block sm:flex text-sm mt-6 lg:mt-0">
						...
						{navigation.map((item, index) => (
							<li key={item.id}>
								<a
									...
									className={`inline-block py-2 px-3 text-gray-500 hover:cursor-pointer hover:text-white no-underline ${item.active ? "text-white" : ""}`}
								>
									{item.name}
								</a>
							</li>
					...
					<ul className="text-gray-700 list-none p-0 font-thin flex flex-col text-left w-full">
						...
						<li>
							<a className="inline-block py-2 px-3 text-gray-500 hover:text-white no-underline">
								Privacy
							</a>
						</li>
						<li>
			...
			<div className="mt-4 pt-6 text-gray-600 border-t border-gray-800 text-center">
				{" "}
				© 2025 Zebook. All rights reserved.
			</div>
		</footer>
	);
}
